    ...
    var computeAggregateCensusMetrics = function(objDict) {
      ...
      for (var key in objDict) {
        if(objDict.hasOwnProperty(key)) {
          ...
          if( caNumber != NaN) {
            sumCensusObj += 1;
            sumPercentHousingCrowded += parseFloat(objDict[key].percentHousingCrowded);
            sumPercentHouseholdsBelowPoverty += parseFloat(objDict[key].percentHouseholdsBelowPoverty);
            sumPercentAged16Unemployed += parseFloat(objDict[key].percentAged16Unemployed);
            sumPercentAged25NoDiploma += parseFloat(objDict[key].percentAged25NoDiploma);
            sumPercentAgedUnder18orOver64 += parseFloat(objDict[key].percentAgedUnder18orOver64);
            sumPerCapitaIncome += parseInt(objDict[key].perCapitaIncome);
            sumHardshipIndex += parseInt(objDict[key].hardshipIndex);
